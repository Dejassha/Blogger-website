...
def loginindex():
    cur=mysql.connection.cursor()
    cur.execute("select Name,bname,bimg,desp,bdate from tbl_blog,details where tbl_blog.did=details.did")
    data=cur.fetchall()
...
def addblogform():
    ...
    if request.method=='POST': 
        ...
        if file and allowed_extensions(file.filename):
            ...
            cur=mysql.connection.cursor()
            cur.execute("INSERT INTO tbl_blog(bname,bimg,desp,did) VALUES  (%s,%s,%s,%s)", (bname,new_filename,desp,session['did']))
...
def uploadimg():
    if request.method=='POST':
        ...
        if file and allowed_extensions(file.filename):
            ...
            cur = mysql.connection.cursor()
            cur.execute("Update details set image=%s where did=%s ",(new_filename,session['did']))
            mysql.connection.commit()
            cur.close()
